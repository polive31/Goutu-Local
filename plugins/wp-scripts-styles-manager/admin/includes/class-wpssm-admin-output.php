	...
	protected function form_buttons($referer) { 
	  ...
	  <table class="button-table" col="2">
	    <tr>
				...
				<?php		
				WPSSM_Debug::log( 'In WPSSM_Admin_Output Nonce : ' . $this->nonce . ' Action : ' . $this->form_action );
...
--------------------------------------------------------------------*/
	...
	public function toggle_switch_recording_cb() {
		$this->toggle_switch( 'general_record', $this->record );
	}	

	public function toggle_switch_optimize_cb() {
		$this->toggle_switch( 'general_optimize', $this->optimize );
	}		
	
	public function toggle_switch_javasync_cb() {
		$this->toggle_switch( 'general_javasync', $this->javasync );
	}	
	...
	public function item_content( $location, $handle ) {
		...
		?>
	    	...
	    	<td class="location <?php echo $this->classmod( $handle, 'location');?>">
	    		<select data-dependencies='<?php echo json_encode($dependencies);?>' data-dependents='<?php echo json_encode($dependents);?>' id="<?php echo $handle;?>" class="asset-setting location <?php echo $this->type;?>" name="<?php echo $this->get_input_name($type, $handle, 'location');?>">
	    		...
	    		<select id="<?php echo $handle;?>" class="asset-setting minify <?php echo $this->type;?>" <?php echo ($asset_is_minified)?'disabled':'';?> title="<?php echo ($asset_is_minified)?$already_minified_msg:'';?>" name="<?php echo $this->get_input_name($type, $handle, 'minify');?>">
