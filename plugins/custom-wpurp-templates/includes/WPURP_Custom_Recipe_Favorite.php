...
class WPURP_Custom_Recipe_Favorite extends WPURP_Template_Block {
    ...
    public function output( $recipe, $args = array() )
    {
        ...
        if( WPURP_Favorite_Recipes::is_favorite_recipe( $recipe->ID() ) ) {
        	$title=__('In my favorites','foodiepro');
        	$class = 'is-favorite';
        }
				else {
					$title=__('Add to my favorites','foodiepro');
					$class='';
				}
        ...
        ob_start();
?>
				<a href="#"<?php echo $this->style(); ?> class="<?php echo $class ?>" title="<?php echo $title?>" data-recipe-id="<?php echo $recipe->ID(); ?>">
