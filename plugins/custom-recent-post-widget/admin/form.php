...
<div class="rpwe-columns-3">
	...
	<div class="rpwe-multiple-check-form">
		<label>
			<?php _e('Display overlays', 'recent-posts-widget-extended'); ?>
		</label>
		<ul>
			<?php foreach (rpwe_tax_list() as $tax_overlay) : ?>
				<li>
					<input type="checkbox" value="<?php echo $tax_overlay; ?>" id="<?php echo $this->get_field_id('tax-overlay') . '-' . $tax_overlay; ?>" name="<?php echo $this->get_field_name('tax-overlay'); ?>[]" <?php checked(is_array($instance['tax-overlay']) && in_array($tax_overlay, $instance['tax-overlay'])); ?> />
					<label for="<?php echo $this->get_field_id('tax-overlay') . '-' . $tax_overlay; ?>">
						<?php echo esc_html($tax_overlay); ?>
					</label>
				</li>
			<?php endforeach; ?>
		</ul>
