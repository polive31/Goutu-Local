...
//	);
	...
	public function jco_settings_init() {
	    ...
	    add_settings_field(
	        ...
	        'js_css_optimization',
	        'enqueued_list_section',
	        array( 'label_for' => 'jco-enqueued-scripts' )
	    );
	        ...
	        'js_css_optimization',
	        'enqueued_list_section',
	        array( 'label_for' => 'jco-enqueued-styles' )
	...
	public function record_enqueued_assets( $in_footer ) {
	  ...
	  PC::debug(array('assets before update' => $this->enqueued_assets));
		...
		foreach( $scripts as $handle ) {
			...
			$this->enqueued_assets['scripts'][$handle]['minify']=(strpos( $obj->src, '.min.' ))?'no':'yes';
			PC::debug( array('$this->enqueued_assets[scripts][$handle]'=>$this->enqueued_assets['scripts'][$handle]) );
		}
		...
		foreach( $styles as $handle ) {
			...
			$this->enqueued_assets['styles'][$handle]['minify']=(strpos( $obj->src, '.min.' ))?'no':'yes';
		}
