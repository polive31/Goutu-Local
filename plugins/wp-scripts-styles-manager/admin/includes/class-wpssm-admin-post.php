...
class WPSSM_Admin_Post {
  ...
  public function __construct( $args ) {
 		PHP_Debug::log('*** In WPSSM_Admin_Post __construct ***' );		  	 	
  	$this->hydrate_args( $args );
  	$this->args = $args;
 		$this->type = $this->get_tab();
 		PHP_Debug::log('In WPSSM_Admin_Post __construct $this->type = ' . $this->type );		  	 	
  }
 ...
 public function init_post_cb() {
 		$this->type = $this->get_tab();
 		PHP_Debug::log('In WPSSM_Admin_Post init_post_cb(), $this->type = ' . $this->type );		  	 	
 }
...
--------------------------------------------------------------*/
	public function update_settings_cb() {
		PHP_Debug::log('In update_settings_cb, type = ' . $this->type );
    ...
    if ( ! wp_verify_nonce( $_POST[ $this->nonce ], $this->form_action ) )
		...
		PHP_Debug::log('Security checks passed');
		...
		if ( isset ( $_POST[ 'wpssm_reset' ] ) ) {
		   	PHP_Debug::log( 'In Form submission : RESET' );
  			$this->Assets 	= new WPSSM_Options_Assets( $this->args );
  			$this->Mods   	= new WPSSM_Options_Mods( $this->args );
				PHP_Debug::log( 'assets before submission' , $this->Assets->get_assets($this->type) );
				$this->Assets->unset_mod( $this->type );
				$this->Mods->reset( $this->type );
				PHP_Debug::log( 'assets after submission', $this->Assets->get_assets($this->type));
		    $query_args['msg']='reset';
		}
		elseif ( isset ( $_POST[ 'wpssm_delete' ] ) ) {
		   	PHP_Debug::log( 'In Form submission : DELETE' );
		...
		elseif ( isset ( $_POST[ 'wpssm_save' ] ) ) {
				PHP_Debug::log( 'In Form submission : SAVE, tab ' . $this->type );
				if ( $this->type=='general' ) {
			  	$this->General 	= new WPSSM_Options_General( $this->args );
					PHP_Debug::log('In WPSSM_Post general save' );
  				...
  				$this->Mods   	= new WPSSM_Options_Mods( $this->args );
					$this->Mods->reset($this->type);	
					PHP_Debug::log( 'assets before submission',$this->Assets->get() );
					PHP_Debug::log( 'mods before submission',$this->Mods->get() );
					foreach ( $this->Assets->get($this->type) as $handle=>$asset ) {
						PHP_Debug::log( array('Looping : asset = ' => $asset ) );
						PHP_Debug::log( array('Looping : handle = ' => $handle ) );
					...
					PHP_Debug::log( 'assets after submission',$this->Assets->get() );				
					PHP_Debug::log( 'mods after submission',$this->Mods->get()) ;				
		...
		PHP_Debug::log('http referer',$url);
		$url = add_query_arg( $query_args, $url) ;
		PHP_Debug::log('url for redirection',$url);
