...
class WPSSM_Options_General extends WPSSM_Options {
	...
	public function __construct( $args ) {
		PHP_Debug::log('*** In WPSSM_Options_General __construct ***' );
		...
		//PHP_Debug::log('In WPSSM_Assets __construct() $this->get()', $this->get() );
		//PHP_Debug::log('In WPSSM_Assets __construct() $this->get(record)', $this->get('record') );
	}
	...
	public function update_from_post() {
		PHP_Debug::log('In WPSSM_Options_General update_from_post' );
		foreach ($this->opt_proto as $setting=>$value) {
			...
			if ( isset($_POST[ $name ]) )
				...
				$value='off';
			PHP_Debug::log('In WPSSM_Options_General update_from_post loop ' . $setting . ' = ' . $value  );
